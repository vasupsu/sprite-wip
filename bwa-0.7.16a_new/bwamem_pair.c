...
void mem_aln2sam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, fullRec *fR, otherRec *oR, bseq1_t *s, int n, const mem_aln_t *list, int which, const mem_aln_t *m);
...
int mem_sam_pe(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], uint64_t id, bseq1_t s[2], mem_alnreg_v a[2])
{
	...
	if (n_pri[0] && n_pri[1] && (o = mem_pair(opt, bns, pac, pes, s, a, id, &subo, &n_sub, z, n_pri)) > 0) {
		...
		fR.pos = oR.pos = -1;
		for (i = 0; i < 1/*n_aa[0]*/; ++i)//vasu
			mem_aln2sam(opt, bns, &str, &fR, &oR, &s[0], n_aa[0], aa[0], i, &h[1]); // write read1 hits
		s[0].sam = strdup(str.s); str.l = 0;
		bseq1_t *myS = &s[0];
		if (fR.pos != -1)
		{
			myS->oR=NULL;
			myS->fR = (fullRec *)malloc (sizeof(fullRec));
			assert (myS->fR != NULL);
			memcpy (myS->fR, &fR, sizeof(fullRec));
		}
		else if (oR.pos != -1)
		{
			myS->fR=NULL;
			myS->oR = (otherRec *)malloc (sizeof(otherRec));
			assert (myS->oR != NULL);
			memcpy (myS->oR, &oR, sizeof(otherRec));
		}
		fR.pos = oR.pos = -1;
		for (i = 0; i < 1/*n_aa[1]*/; ++i)
			mem_aln2sam(opt, bns, &str, &fR, &oR, &s[1], n_aa[1], aa[1], i, &h[0]); // write read2 hits
		s[1].sam = str.s;
		myS = &s[1];
		if (fR.pos != -1)
		{
