...
void mem_aln2sam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, fullRec *fR, otherRec *oR, bseq1_t *s, int n, const mem_aln_t *list, int which, const mem_aln_t *m_)
{
	int i, l_name;
...
//	printf ("%s\n", s->name);
	...
	if (p->rid >= 0) { // with coordinate
		...
		if ((p->n_cigar == 1) && ((p->cigar[0]&0xf)==0) && ((p->cigar[0]>>4) == s->l_seq)) isFr = 1;
		s->contig = p->rid;
//		printf ("%s - nCigar %d cigarOp[0]=%d cigarLen[0]=%d, seq.l %d FR %d contig %d\n", s->name, p->n_cigar, p->cigar[0]&0xf, p->cigar[0]>>4, s->l_seq, isFr, s->contig);
		if (isFr)
		{
			fR->flag = (p->flag&0xffff) | (p->flag&0x10000? 0x100 : 0);
			fR->pos = p->pos + 1;
			fR->qual = p->mapq;
			fR->matchLen = s->l_seq;
		}
		...
		{
			oR->flag = (p->flag&0xffff) | (p->flag&0x10000? 0x100 : 0);
			oR->pos = p->pos + 1;
			oR->qual = p->mapq;
			oR->n_cigar = (p->n_cigar > 10)? 10 : p->n_cigar;
			int i;
			for (i=0; i<p->n_cigar; i++)
			{
				oR->cigar[i]=(uint16_t)p->cigar[i];
				if (i==9) break;
			}
		}
	...
	if (p->flag & 0x100) { // for secondary alignments, don't write SEQ and QUAL
		...
		for (i = qb; i < qe; ++i) 
		{
			...
			{
				if (subInd==0)
					fR->seq[seqInd] = bToInt[(int)s->seq[i]]<<4;
				else
					fR->seq[seqInd] |= bToInt[(int)s->seq[i]];
				fR->quals[j]=s->qual[i]-33;
			}
			else
			{
				if (subInd==0)
					oR->seq[seqInd] = bToInt[(int)s->seq[i]]<<4;
				else
					oR->seq[seqInd] |= bToInt[(int)s->seq[i]];
				oR->quals[j]=s->qual[i]-33;
			}
			j++;
		}
		kputc('\t', str);
...
void mem_reg2sam(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a, int extra_flag, const mem_aln_t *m)
{
	...
	if (aa.n == 0) { // no alignments good enough; then write an unaligned record
		...
		t.flag |= extra_flag;
		mem_aln2sam(opt, bns, &str, &fR, &oR, s, 1, &t, 0, m);
	} else {
		for (k = 0; k < 1/*aa.n*/; ++k)
			mem_aln2sam(opt, bns, &str, &fR, &oR, s, aa.n, aa.a, k, m);
	...
	{
		s->oR=NULL;
		s->fR = (fullRec *)malloc (sizeof(fullRec));
		assert (s->fR != NULL);
		memcpy (s->fR, &fR, sizeof(fullRec));
	}
	else if (oR.pos != -1)
	{
		s->fR=NULL;
		s->oR = (otherRec *)malloc (sizeof(otherRec));
		assert (s->oR != NULL);
		memcpy (s->oR, &oR, sizeof(otherRec));
	}
	else
	{
		s->fR=NULL;
...
static void worker2(void *data, int i, int tid)
{
	...
	if (!(w->opt->flag&MEM_F_PE)) {
		...
		mem_sam_pe(w->opt, w->bns, w->pac, w->pes, (w->n_processed>>1) + i, &w->seqs[i<<1], &w->regs[i<<1]);//vasu
