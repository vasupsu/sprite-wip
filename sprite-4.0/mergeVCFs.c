...
int main(int argc, char **argv)
{
	...
	while (!feof (fpStrelka))
	{
		if (strelka_line[0] != '#')
		{
			...
			{
				while (!feof (fpParsnip) && ((strcmp(parsnip_contigstr, strelka_contigstr) && (parsnip_pos > strelka_pos)) || (!strcmp(parsnip_contigstr, strelka_contigstr) && (parsnip_pos < strelka_pos))))
				{
